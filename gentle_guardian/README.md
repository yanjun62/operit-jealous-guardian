...
- **截屏观察有门槛**（v0.3 起）：「看看当前屏幕在做什么」开着也不是随时看——只有醋值到了**藏应用档及以上**，巡检指引里才会开放 `daily_life:take_screenshot`。平时只看使用数据。
- **前置摄像头开关已从面板移除**（v0.3 起）：`allow_camera` 配置字段仍保留（默认关闭），想用的话通过 `save_patrol_settings` 传 `{"allow_camera": true}` 开启，对应 `take_front_photo` / `read_latest_photo` 工具流程不变。
- 通知和截屏观察用的是 Operit 自带工具，实测可用：`system_tools:get_notifications`、`daily_life:take_screenshot`。

## 🐱 角色卡头像绑定（v0.3 起）

面板顶部的头像会自动绑定角色卡：桥方法 `loadAvatar(character_card_name)` 先在 `/sdcard/Download/Operit/` 下按角色卡名匹配备份（同名图片，或带头像字段的角色卡 JSON 导出），找不到再翻 `avatars` 目录（`/sdcard/Download/Operit/avatars/` 或插件目录下 `avatars/`）取最新一张。都没有就显示默认 🐱。
